...
class AdsController extends Module{
    ...
    public function clickgo($params){
        ...
        if(isset($logged)){
            ...
            $ads = $db->super_query("SELECT typepay, click, active, price, link FROM `".PREFIX."_ads` WHERE id = '{$aid}'");
            if($ads){
                $db->query("UPDATE `".PREFIX."_ads` SET click = click+1 WHERE id = '{$aid}'");
                if($ads['typepay'] == 1){
                    $db->query("UPDATE `".PREFIX."_ads` SET price = price-1 WHERE id = '{$aid}'");
                    if($ads['price']-1 == 0){
                        $db->query("UPDATE `".PREFIX."_ads` SET active = 0 WHERE id = '{$aid}'");
                    }
                }
                echo $ads['link'];
            }
    ...
    public function status_ad($params){
        ...
        if(isset($logged)){
            ...
            $aid = intval($_POST['aid']);
            $row = $db->super_query("SELECT typepay, active FROM `".PREFIX."_ads` WHERE author = '{$user_id}' AND id = '{$aid}'");
            if($row){
                if($row['active'] == 1){
                    $newactive = 0;
                    $texta = 'остановлено.';
                ...
                $mybalance = $db->super_query("SELECT balance_rub FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
                if($row['typepay'] == 1){
                    if($mybalance['balance_rub'] > 4){
                        $db->query("UPDATE `".PREFIX."_ads` SET active = '{$newactive}' WHERE id = '{$aid}'");
    ...
    public function cabinet($params){
        ...
        if(isset($logged)){
            ...
            $mybalance = $db->super_query("SELECT balance_rub FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
            $myads = $db->super_query("SELECT COUNT(*) AS cnt FROM `".PREFIX."_ads` WHERE author = '{$user_id}'");
            $tpl->load_template('ads/cabinet.tpl');
            $tpl->set('{balance}', $mybalance['balance_rub']);
            if($myads['cnt']){
                $sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS * FROM `".PREFIX."_ads` WHERE author = '{$user_id}' ORDER by `views` DESC", 1);
                $myads2 = '';
                foreach($sql_ as $row){
                    if($row['active'] == '1'){
                        $active = 'status_on';
    ...
    public function optionad($params){
        ...
        if(isset($logged)){
            ...
            if($whad == 'url'){
                ...
                $sql_app = $db->super_query("SELECT tb1.game_id, tb2.title, traf, poster FROM `".PREFIX."_games_users` tb1, `".PREFIX."_games` tb2 WHERE tb1.game_id = tb2.id AND tb1.user_id = '{$user_id}' ORDER by `lastdate` DESC LIMIT 0, 50", 1);
                ...
                $sql_groups = $db->super_query("SELECT SQL_CALC_FOUND_ROWS id, title, traf, photo, ulist FROM `".PREFIX."_communities` WHERE ulist regexp '[[:<:]]({$user_id})[[:>:]]' ORDER by `traf` DESC LIMIT 0, 50", 1);
    ...
    public function bigtype($params){
        ...
        if(isset($logged)){
            ...
            if($type == 3){
                $public = $db->super_query("SELECT title, traf, photo FROM `".PREFIX."_communities` WHERE id = '{$publicid}'");
                if($public){
                ...
                $app = $db->super_query("SELECT title, poster, traf FROM `".PREFIX."_games` WHERE id = '{$appid}'");
                if($app){
    ...
    public function createad($params){
        ...
        if(isset($logged)){
            ...
            if($text && $whads && $type && $typepay && $link && $mybalance['balance_rub'] >= $bal){
                if($whads == 'url'){
                    $whads = 1;
                    $db->query("INSERT INTO `".PREFIX."_ads` SET `text` = '{$text}', `description` = '{$description}', `whads` = '{$whads}', `type` = '{$type}', `typepay` = '{$typepay}', `image` = '{$image}', `link` = '{$link}', `price` = '{$price}', `author` = '{$user_id}', `balance` = '{$bal}'");
                    $adid = $db->insert_id();
                    $db->query("INSERT INTO `".PREFIX."_ads_settings` SET `idad` = '{$adid}', `country` = '{$country}', `sex` = '{$sex}', `agef` = '{$agefrom}', `agel` = '{$agelast}', `sp` = '{$sp}'");
                    $result = 'success';
                } elseif($whads == 'pub'){
                    $whads = 3;
                ...
                $db->query("UPDATE `".PREFIX."_users` SET balance_rub = balance_rub-{$bal} WHERE user_id = '{$user_id}'");
            }
            echo $result;
            die();
    ...
    public function view_ajax($params){
        ...
        if(isset($logged)){
            ...
            //$ad = $db->super_query("SELECT * FROM `".PREFIX."_ads` WHERE price != '0' and active = '1' ORDER BY RAND() LIMIT 5");
            $ad = $db->super_query("SELECT tb1.whads, id, link, image, text, description, type, views, price, active, tb2.idad, country, city, sex, agef, agel, sp FROM `".PREFIX."_ads` tb1, `".PREFIX."_ads_settings` tb2 WHERE (tb1.price != '0' and tb1.active = '1' AND tb1.id = tb2.idad) AND ((tb2.country = '{$user_info['user_country']}' OR tb2.country = 0) AND (tb2.sex = 0 OR tb2.sex = '{$user_sex}') AND (tb2.agef >= '{$user_age}' OR tb2.agef = 0) AND (tb2.agel <= '{$user_age}' OR tb2.agel = 0)) ORDER BY RAND() LIMIT 5");
            if($ad){
                $traf = '';
                $temp = '';
                if($ad['whads'] == 1){
                    ...
                    $row = $db->super_query("SELECT traf FROM `".PREFIX."_communities` WHERE id = '{$pid}'");
                    $traf = $row['traf'];
                } elseif($ad['whads'] == 2){
                    $temp = 'app';
                    $aid = substr($ad['link'], 3);
                    $row = $db->super_query("SELECT traf FROM `".PREFIX."_games` WHERE id = '{$aid}'");
                    $traf = $row['traf'];
                }
                $db->query("UPDATE `".PREFIX."_ads` SET views = views+1 WHERE id = '{$ad['id']}'");
                if($ad['typepay'] == 2){
                    $db->query("UPDATE `".PREFIX."_ads` SET price = price-1 WHERE id = '{$ad['id']}'");
                    if($ad['price']-1 == 0){
                        $db->query("UPDATE `".PREFIX."_ads` SET active = 0 WHERE id = '{$ad['id']}'");
                    }
                }
