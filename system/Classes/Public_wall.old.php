...
//		global $tpl;
	...
	function compile($compile, $tpl){
		$this->compile = $compile;
        return $tpl;
	}
	
	function select($public_admin, $server_time, $tpl){
		global $row;
				...
				foreach($attach_arr as $attach_file){
					...
					//Если опрос
						...
						if($vote_id){
							...
							if($row_vote['answer_num']) $answer_num_text = System\Libs\Gramatic::gram_record($row_vote['answer_num'], 'fave');
...
HTML;
			...
			if($row_wall['likes_num']){
				...
				$tpl->set('{likes-text}', '<span id="like_text_num'.$row_wall['id'].'">'.$row_wall['likes_num'].'</span> '.System\Libs\Gramatic::gram_record($row_wall['likes_num'], 'like'));
			} else {
				$tpl->set('{likes}', '');
			...
			if($row_wall['comments'] OR $public_admin){
				if($row_wall['fasts_num']){
					...
					$tpl->set('{gram-record-all-comm}', System\Libs\Gramatic::gram_record(($row_wall['fasts_num']-3), 'prev').' '.($row_wall['fasts_num']-3).' '.System\Libs\Gramatic::gram_record(($row_wall['fasts_num']-3), 'comments'));
