...
class ResultSet implements \Iterator
{
	...
	public function normalizeRow(array $row): array
	{
		...
		foreach ($this->types as $key => $type) {
			...
			if ($value === null || $value === false || $type === IStructure::FIELD_TEXT) {
				...
				$row[$key] = new Sura\Utils\DateTime($value);
				...
				$row[$key] = Sura\Utils\DateTime::from($value);
			}
		}
	...
	public function rewind(): void
	{
		if ($this->result === false) {
			throw new Sura\InvalidStateException(self::class . ' implements only one way iterator.');
		}
	}
	...
	{
		return Sura\Utils\Arrays::associate($this->fetchAll(), $path);
	}
}
