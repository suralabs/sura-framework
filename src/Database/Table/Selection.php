...
class Selection implements \Iterator, \ArrayAccess, \Countable
{
	...
	public function __construct(
		...
		string $tableName,
		Sura\Caching\IStorage $cacheStorage = null
	) {
		...
		$this->cache = $cacheStorage
			? new Sura\Caching\Cache($cacheStorage, 'Sura.Database.' . md5($explorer->getConnection()->getDsn()))
			: null;
	...
	{
		return Sura\Database\Helpers::toPairs($this->fetchAll(), $key, $value);
	}
	...
	public function fetchAssoc(string $path): array
	{
		...
		return Sura\Utils\Arrays::associate($rows, $path);
	}
	...
	{
		if (is_array($this->primary) && Sura\Utils\Arrays::isList($key)) {
		...
		} elseif (is_array($key) && !Sura\Utils\Arrays::isList($key)) { // key contains column names
			$this->where($key);
		} else {
	 ...
	 * @return static
	 * @throws \Sura\InvalidArgumentException
	 */
		...
		foreach ($parameters as $key => $val) {
			if (is_int($key)) { // whereOr(['full condition'])
				...
				if ($qNumber > 1 && (!is_array($val) || $qNumber !== count($val))) {
					throw new Sura\InvalidArgumentException('Argument count does not match placeholder count.');
				}
				$columns[] = $key;
	...
	protected function execute(): void
	{
		...
		if ($this->primary === null && $this->sqlBuilder->getSelect() === null) {
			throw new Sura\InvalidStateException('Table with no primary key requires an explicit select clause.');
		}

		try {
			$result = $this->query($this->getSql());

		} catch (Sura\Database\DriverException $exception) {
	...
	protected function query(string $query): Sura\Database\ResultSet
	{
	...
	public function update(iterable $data): int
	{
		if ($data instanceof \Traversable) {
			...
			throw new Sura\InvalidArgumentException;
		}
