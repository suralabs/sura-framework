...
namespace Sura\Database\Table;

use Sura;
use Sura\Database\Conventions;
use Sura\Database\Driver;
use Sura\Database\Explorer;
use Sura\Database\IStructure;
use Sura\Database\SqlLiteral;
...
class SqlBuilder
{
	...
	{
		if (is_array($columns)) {
			throw new Sura\Exception\InvalidArgumentException('Select column must be a string.');
		}
		$this->select[] = $columns;
	...
	protected function addCondition($condition, array $params, array &$conditions, array &$conditionsParameters): bool
	{
		...
		if ($placeholderCount === 0 && count($params) === 1) {
			...
			throw new Sura\Exception\InvalidArgumentException('Argument count does not match placeholder count.');
		}
			...
			if ($arg === null) {
				...
				if ($hasOperator) {
					if (trim($match[2][0]) === 'NOT') {
						...
						throw new Sura\Exception\InvalidArgumentException('Column operator does not accept null argument.');
					}
				}
				...
				if ($hasOperator) {
					if (trim($match[2][0]) === 'NOT') {
						...
						throw new Sura\Exception\InvalidArgumentException('Column operator does not accept array argument.');
					}
				} else {
					$match[2][0] = 'IN ';
				}
				...
				if ($arg instanceof Selection) {
					...
					if (!$clone->getSqlBuilder()->select) {
						try {
							...
							throw new Sura\Exception\InvalidArgumentException('Selection argument must have defined a select column.', 0, $e);
						}
					}
				...
				if ($arg !== null) {
					if (!$arg) {
						...
						if (!$hasBrackets && ($hasOperators || ($hasNot && !$hasPrefixNot))) {
							throw new Sura\Exception\InvalidArgumentException('Possible SQL query corruption. Add parentheses around operators.');
						}
	...
	protected function checkUniqueTableName(string $tableName, string $chain): void
	{
		...
		if (isset($this->reservedTableNames[$tableName])) {
			...
			throw new Sura\Exception\InvalidArgumentException("Table alias '$tableName' from chain '$chain' is already in use by chain '{$this->reservedTableNames[$tableName]}'. Please add/change alias for one of them.");
		}
	...
	protected function getSortedJoins(string $table, &$leftJoinDependency, &$tableJoins, &$finalJoins): void
	{
		if (isset($this->expandingJoins[$table])) {
			...
			throw new Sura\Exception\InvalidArgumentException("Circular reference detected at left join conditions (tables '$path').");
		}
	...
	public function parseJoinsCb(&$joins, $match): string
	{
		...
		if ($keyMatches[0]['del'] === '.') {
			if (
				...
				throw new Sura\Exception\InvalidArgumentException("Do not prefix table chain with origin table name '{$keyMatches[0]['key']}'. If you want to make self reference, please add alias.");
			}
		...
		foreach ($keyMatches as $index => $keyMatch) {
			...
			if (!$index && isset($this->aliases[$keyMatch['key']])) {
				if ($keyMatch['del'] === ':') {
					throw new Sura\Sura\Exception\InvalidArgumentException("You are using has many syntax with alias (':{$keyMatch['key']}'). You have to move it to alias definition.");
				}
				...
				if (isset($keyMatch['throughColumn'])) {
					...
					if (!$belongsTo) {
						throw new Sura\Exception\InvalidArgumentException("No reference found for \${$parent}->{$keyMatch['key']}.");
					}
					[, $primary] = $belongsTo;
					...
					if (!$hasMany) {
						throw new Sura\Exception\InvalidArgumentException("No reference found for \${$parent}->related({$keyMatch['key']}).");
					}
	...
	protected function addConditionComposition(
		...
		if ($this->driver->isSupported(Driver::SUPPORT_MULTI_COLUMN_AS_OR_COND)) {
			...
			return $this->addCondition($condition, [Sura\Utils\Arrays::flatten($parameters)], $conditions, $conditionsParameters);
		} else {
