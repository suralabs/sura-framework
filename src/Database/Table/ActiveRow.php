...
class ActiveRow implements \IteratorAggregate
{
	 ...
	 * @return mixed possible int, string, array, object (Sura\Utils\DateTime)
	 */
	public function getPrimary(bool $throw = true)
	{
		...
		if ($primary === null) {
			...
			if (isset($this->data[$primary])) {
				...
				throw new Sura\Exception\InvalidStateException("Row does not contain primary $primary column data.");
			} else {
				return null;
			}
			...
			foreach ($primary as $key) {
				if (!isset($this->data[$key])) {
					if ($throw) {
						throw new Sura\Exception\InvalidStateException("Row does not contain primary $key column data.");
					} else {
						return null;
					}
				}
	...
	public function ref(string $key, string $throughColumn = null): ?self
	{
		...
		if ($row === false) {
			throw new Sura\MemberAccessException("No reference found for \${$this->table->getName()}->ref($key).");
		}
	...
	public function related(string $key, string $throughColumn = null): GroupedSelection
	{
		...
		if (!$groupedSelection) {
			throw new Sura\Exception\MemberAccessException("No reference found for \${$this->table->getName()}->related($key).");
		}
	...
	public function update(iterable $data): bool
	{
		...
		if ($selection->update($data)) {
			...
			if (($row = $selection->fetch()) === null) {
				throw new Sura\Exception\InvalidStateException('Database refetch failed; row does not exist!');
			}
			$this->data = $row->data;
			return true;
		} else {
	...
	{
		throw new Sura\Exception\DeprecatedException('ActiveRow is read-only; use update() method instead.');
	}
	...
	public function &__get(string $key)
	{
		...
		$hint = Sura\Utils\Helpers::getSuggestion(array_keys($this->data), $key);
		throw new Sura\MemberAccessException("Cannot read an undeclared column '$key'" . ($hint ? ", did you mean '$hint'?" : '.'));
	}
	...
	{
		throw new Sura\Exception\DeprecatedException('ActiveRow is read-only.');
	}
	...
	public function accessColumn($key, bool $selectColumn = true): bool
	{
		if ($this->table->accessColumn($key, $selectColumn) && !$this->dataRefreshed) {
			if (!isset($this->table[$this->getSignature()])) {
				throw new Sura\Exception\InvalidStateException("Database refetch failed; row with signature '{$this->getSignature()}' does not exist!");
			}
