...
class Image
{
	...
	public static function fromFile(string $file, int &$type = null): static
    {
		...
		if (!$type) {
			throw new Sura\Exception\UnknownImageFileException(is_file($file) ? "Unknown type of file '$file'." : "File '$file' not found.");
		}

		$method = 'imagecreatefrom' . self::FORMATS[$type];
		return new static(Callback::invokeSafe($method, [$file], function (string $message): void {
			throw new Sura\Exception\ImageException($message);
		}));
	}
	...
	public static function fromString(string $s, int &$type = null): static
    {
		...
		if (!$type) {
			throw new Sura\Exception\UnknownImageFileException('Unknown type of image.');
		}
	...
	public static function fromBlank(int $width, int $height, array $color = null): static
    {
		...
		if ($width < 1 || $height < 1) {
			throw new Sura\Exception\InvalidArgumentException('Image width and height must be greater than zero.');
		}
	...
	public static function typeToExtension(int $type): string
	{
		if (!isset(self::FORMATS[$type])) {
			throw new Sura\Exception\InvalidArgumentException("Unsupported image type '$type'.");
		}
	...
	public static function calculateSize(
		...
		if ($flags & self::STRETCH) { // non-proportional
			if (!$newWidth || !$newHeight) {
				throw new Sura\Exception\InvalidArgumentException('For stretching must be both width and height specified.');
			}
			...
			if (!$newWidth && !$newHeight) {
				throw new Sura\Exception\InvalidArgumentException('At least width or height must be specified.');
			}
	...
	public function save(string $file, int $quality = null, int $type = null): void
	{
		if ($type === null) {
			...
			if (!isset($extensions[$ext])) {
				throw new Sura\Exception\InvalidArgumentException("Unsupported file extension '$ext'.");
			}
			$type = $extensions[$ext];
		}
	...
	private function output(int $type, ?int $quality, string $file = null): void
	{
		...
		if (!$success) {
			throw new Sura\Exception\ImageException(Helpers::getLastError() ?: 'Unknown error');
		}
	}
	...
	private static function isPercent(int|string $num): bool
	{
		...
		throw new Sura\Exception\InvalidArgumentException("Expected dimension in int|string, '$num' given.");
	}
	...
	{
		throw new Sura\Exception\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
	}
}
